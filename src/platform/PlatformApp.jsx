...
function NewStory({ session }) {
  ...
  return (
    <div className="riu-page create-page"><Header session={session} /><main className="create-layout"><section><span className="riu-overline">Neue Story</span><h1>Ein Ort für Ihre Erzählung.</h1><p>Beginnen Sie mit einem einzelnen 3D-Modell oder erzählen Sie mit mehreren Stationen als fließende Scrolling-Story.</p><div className="cors-note"><ExternalLink size={19} /><div><strong>Mit eigenem Modell</strong><span>Bei glTF-, GLB- und FBX-Dateien muss der Modellserver Browserzugriffe per CORS erlauben. Bei Sketchfab genügt die kopierte URL der Modellseite.</span></div></div></section><form className="create-form" onSubmit={submit}><label>Titel der Story<input required name="name" placeholder="Zum Beispiel: Spuren einer Stadt" /></label><label>Kurzbeschreibung<textarea required name="description" rows="4" placeholder="Worum geht es in dieser Scrolling-Erzählung?" /></label><div className="create-form-row"><label>Sprache<select required name="language" defaultValue="de">{Object.entries(STORY_LANGUAGES).map(([code, label]) => <option value={code} key={code}>{label}</option>)}</select></label><CategoryPicker /></div><label>URL zum 3D-Modell <span>(optional)</span><input name="modelUrl" type="url" placeholder="https://sketchfab.com/3d-models/… oder https://…/modell.glb" /><small>Sketchfab-Modellseite einfach kopieren und einfügen. Alternativ: FBX, GLB oder glTF über HTTP(S).</small><span className="model-room-hint"><Box size={16} /><span><strong>Kein Modell zur Hand?</strong>Lassen Sie das Feld leer. RIU erstellt dann eine Scrolling-Ausstellung, in der Sie mehrere Stationen und Objekte anordnen können.</span></span></label><label>Vorschaubild URL <span>(optional)</span><input name="coverImage" type="url" placeholder="https://example.org/cover.jpg" /></label>{error && <div className="form-error">{error}</div>}<div className="create-actions"><button type="button" onClick={() => go('/dashboard')}>Abbrechen</button><button className="riu-button">Story anlegen <ArrowRight size={17} /></button></div></form></main></div>
  );
}
